...
content = content.replace(/<DropdownPopover[\s\S]*?aria-label="Insert Menu"[\s\S]*?<\/DropdownPopover>/, insertMenu);
content = content.replace(/<DropdownPopover[\s\S]*?aria-label="Format Menu"[\s\S]*?<\/DropdownPopover>/, formatMenu);
content = content.replace(/<DropdownPopover[\s\S]*?aria-label="File Menu"[\s\S]*?<\/DropdownPopover>/, fileMenu);
content = content.replace(/<DropdownPopover[\s\S]*?aria-label="Edit Menu"[\s\S]*?<\/DropdownPopover>/, editMenu);
content = content.replace(/<DropdownPopover[\s\S]*?aria-label="View Menu"[\s\S]*?<\/DropdownPopover>/, viewMenu);
content = content.replace(/<DropdownPopover[\s\S]*?aria-label="Tools Menu"[\s\S]*?<\/DropdownPopover>/, toolsMenu);

fs.writeFileSync('components/creonity-docs/docs-header.tsx', content);
console.log('Fixed menus according to instructions!');
