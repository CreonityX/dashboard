...
import {
  ...
  triggerSocialSyncAction,
} from "@/app/actions/social";
import { toast } from "sonner";
import { SettingsPage, SettingsSection, SettingsCard } from "../settings-ui";
...
const INTEGRATIONS = [
  {
    ...
    desc: "Sync your follower count and latest posts.",
    status: "connected",
    meta: "@alexrivera",
  },
  {
    ...
    desc: "Showcase your viral videos and engagement.",
    status: "connected",
    meta: "@arivera",
  },
  {
    ...
    desc: "Display your channel analytics and videos.",
    status: "disconnected",
    meta: null,
  },
  {
    ...
    desc: "Track traffic to your creator portfolio.",
    status: "disconnected",
    meta: null,
  },
  {
    ...
    desc: "Sync tasks and campaign notes.",
    status: "disconnected",
    meta: null,
  },
  {
  ...
  },
  {
    ...
    desc: "Sync campaign deadlines and calls.",
    status: "connected",
    meta: "alex.r@example.com",
  },
  {
    id: "zapier",
    name: "Zapier",
...
export function ConnectedAccountsView({ onBack }: { onBack?: () => void }) {
  ...
  async function handleDisconnect(app: (typeof INTEGRATIONS)[number]) {
    ...
    if (r.success) {
      setSocial((prev) => ({
        ...prev,
        [app.id]: { status: "disconnected", meta: null },
      }));
  ...
  async function handleSync(app: (typeof INTEGRATIONS)[number]) {
    const r = await triggerSocialSyncAction(app.id);
    if (r.success) {
      toast.success(`${app.name} sync started`, {
        description: "Fresh metrics will appear shortly.",
      });
    } else {
      toast.error(r.error);
    }
  ...
  function rowState(app: (typeof INTEGRATIONS)[number]) {
    if (!SOCIAL_IDS.includes(app.id))
      return { status: app.status, meta: app.meta };
      ...
      <SettingsSection>
        <div className="flex flex-col gap-1 max-w-3xl mb-4 px-1">
          <p className="text-[14px] text-[#52525b] dark:text-[#a1a1aa]">
            Connect your favorite tools and social platforms to sync data,
            automate workflows, and enhance your creator portfolio.
          </p>
        </div>
        ...
        <SettingsCard className="flex flex-col p-0 overflow-hidden">
          {INTEGRATIONS.map((app) => {
            ...
            return (
              <div
                key={app.id}
                className="flex items-center justify-between px-6 h-[72px] border-b border-[#f4f4f5] dark:border-[#1f1f1f] last:border-0 gap-4"
              >
                <div className="flex items-center gap-4">
                  <div className="w-8 h-8 flex items-center justify-center shrink-0">
                    <Icon
                      icon={app.icon}
                      className="w-5 h-5 text-[#0a0a0a] dark:text-white"
                    />
                  </div>
                  <span className="text-[15px] font-semibold text-[#0a0a0a] dark:text-white">
                    {app.name}
                  </span>
                </div>
                ...
                {live.status === "connected" ? (
                  ...
                  <Button
                    onClick={() => {
                      if (SOCIAL_IDS.includes(app.id)) void handleConnect(app);
                      else
                        toast.success(`${app.name} connected`, {
                          description: "Account synced successfully.",
                        });
                    }}
                    className="w-[160px] font-medium bg-[#0a0a0a] text-white dark:bg-white dark:text-[#0a0a0a] rounded-full h-10 transition-colors"
                  >
                    Connect
                  </Button>
                )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#111111] rounded-[24px] p-7 w-full max-w-md relative animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-[20px] font-bold text-[#0a0a0a] dark:text-white mb-2">
              Disconnect {disconnectModal.name}?
            </h3>
            <p className="text-[15px] leading-relaxed text-[#52525b] dark:text-[#a1a1aa] mb-8">
              Are you sure you want to disconnect this account from your
              profile?
            </p>

            {SOCIAL_IDS.includes(disconnectModal.id) && (
              <button
                onClick={() => void handleSync(disconnectModal)}
                className="w-full mb-6 px-6 py-2.5 rounded-full bg-[#f4f4f5] hover:bg-[#e4e4e7] dark:bg-[#1f1f1f] dark:hover:bg-[#27272a] text-[#0a0a0a] dark:text-white text-[15px] font-semibold transition-colors"
              >
                Sync now
              </button>
            )}
            ...
            <div className="flex items-center justify-end gap-3">
              <button
                onClick={() => setDisconnectModal(null)}
                className="px-6 py-2.5 rounded-full bg-[#f4f4f5] hover:bg-[#e4e4e7] dark:bg-[#1f1f1f] dark:hover:bg-[#27272a] text-[#0a0a0a] dark:text-white text-[15px] font-semibold transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  if (SOCIAL_IDS.includes(disconnectModal.id))
                    void handleDisconnect(disconnectModal);
                  else {
