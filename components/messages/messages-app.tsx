...
import { type MessageReaction as MessageReactionFromApi } from "@/lib/api";
...
export function MessagesApp() {
  ...
  const [reactions, setReactions] = useState<
    Record<string, MessageReactionFromApi[]>
  >({});
  ...
  const refreshUnread = async () => {
    ...
    setUnread({
      channels: Object.fromEntries(
        res.data.channels.map((c) => [c.id, c.unreadCount]),
      ),
  ...
  const loadMessages = async (convoId: string, channelId?: string | null) => {
    ...
    if (channelId) {
      ...
      // Pre-fetch reactions for every message in the thread so the
      // initial render has them (avoids per-message round-trips).
      void (async () => {
        const settled = await Promise.all(
          rows.map((m) => getMessageReactionsAction(m.id)),
        );
        const next: Record<string, MessageReactionFromApi[]> = {};
        settled.forEach((r, i) => {
          if (r.success) next[rows[i]!.id] = r.data;
        });
  ...
  const handleRealtime = (event: RealtimeEvent) => {
    if (
      ...
      (event.channelId || event.dmThreadId)
    ) {
      ...
      setOnlineByUser((prev) => ({
        ...prev,
        [event.userId as string]: event.status === "online",
      }));
    } else if (
      event.type === "reaction.added" ||
      event.type === "reaction.removed"
    ) {
      const mid = event.messageId as string;
      ...
      setReactions((prev) => {
        ...
        if (event.type === "reaction.added") {
          ...
          if (idx === -1)
            return {
              ...prev,
              [mid]: [...cur, { emoji, count: 1, userIds: [userId] }],
            };
        ...
        return { ...prev, [mid]: cur.filter((r) => r.emoji !== emoji) };
      });
  ...
  const creatorConversations: Conversation[] = !isBrand
    ? [
          ...
          ? [
              {
                ...
                channels: channels.map(
                  (ch: { id: string; name: string; unread?: number }) => ({
                    id: ch.id,
                    name: ch.name,
                ...
                preview: "",
                time: "",
                unread: Object.values(unread.channels).reduce(
                  (n, c) => n + c,
                  0,
                ),
                messages: [],
        ...
        ...threads.map((t) => {
          ...
          const label = other
            ? (other.email.split("@")[0] ?? other.email)
            : "Direct message";
  ...
  const activeChannel = useMemo(
    () =>
      activeChannelId && conversation?.channels
        ? conversation.channels.find(
            (ch: { id: string }) => ch.id === activeChannelId,
          )
        : null,
  ...
  function handleSend(text: string) {
    ...
    if (isBrand && activeConvoId)
      appendBrandMessage(activeConvoId, activeChannelId ?? undefined, message);
    else
      setExtra((prev) => ({
        ...prev,
    ...
    setTimeout(() => {
      ...
      const reply: Message = {
        id: `${activeId}-reply-${Date.now()}`,
        sender: "them",
        time: replyTime,
        kind: "text",
        text: "Got it! I'll look into that and get right back to you.",
      };
      if (isBrand && activeConvoId)
        appendBrandMessage(activeConvoId, activeChannelId ?? undefined, reply);
      else
        setExtra((prev) => ({
          ...prev,
  ...
  function handleReschedule(
    ...
    if (isBrand && activeConvoId)
      appendBrandMessage(activeConvoId, activeChannelId ?? undefined, booking);
    else
      setExtra((prev) => ({
        ...prev,
        [activeId]: [...(prev[activeId] ?? []), booking],
      }));
  }
  ...
  function handleSelect(id: string) {
    if (activeId === "create-workspace" || activeId === "create-group") {
      ...
      const hasModifiedChannels =
        draftState.channels.length > 1 ||
        (draftState.channels.length === 1 &&
          draftState.channels[0].id !== "general");
      if (
        draftState.name ||
        draftState.selectedMembers.length > 0 ||
        hasModifiedChannels
      ) {
  ...
  return (
    <div
      className="flex h-full w-full overflow-hidden bg-white dark:bg-[#0a0a0a]"
      suppressHydrationWarning
    >
      {/* Conversation list — shown on mobile if mobileView is "list", always on desktop */}
      <div
        className={`w-full lg:w-[380px] flex-shrink-0 ${mobileView === "list" ? "block" : "hidden"} lg:block border-r border-[#efefef] dark:border-white/10`}
      >
        <ConversationList
          activeId={activeId || ""}
      ...
      <div
        className={`flex h-full min-w-0 flex-1 flex-col bg-white dark:bg-[#0a0a0a] md:flex ${mobileView === "chat" ? "flex" : "hidden"}`}
      >
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              <div className="flex flex-col">
                ...
                {(() => {
                  const typingKey = activeChannelId
              ...
              <button
                onClick={() => {
                  ...
                  if (pendingSelection !== null) {
                    setActiveId(
                      pendingSelection === "" ? null : pendingSelection,
                    );
