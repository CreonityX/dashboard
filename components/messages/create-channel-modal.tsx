...
const inputClass =
  "h-11 w-full rounded-xl border-none bg-[#f4f4f5] px-4 text-[14px] text-[#0a0a0a] outline-none placeholder:text-[#a1a1aa] focus:ring-2 focus:ring-[#0a0a0a]/10 dark:bg-[#1f1f1f] dark:text-white dark:focus:ring-white/10 transition-all";
...
export function CreateChannelModal({
  ...
  const handleCreate = () => {
    ...
    if (onCreateChannel) {
      if (onCreateChannel(clean, isPrivate)) onClose();
      return;
    }
    createBrandChannel(communityId, clean, {
      private: isPrivate,
      ...
      >
        ...
        {step === 1 ? (
          <div className="flex flex-col gap-4 mt-6">
            <div>
              <label className="mb-2 block text-[13px] font-semibold text-[#0a0a0a] dark:text-white">
                Name
              </label>
              <input
                autoFocus
          ...
          <div className="flex flex-col gap-4 mt-6">
            ...
            >
              <div
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${isPrivate ? "bg-[#0a0a0a] text-white dark:bg-white dark:text-[#0a0a0a]" : "bg-[#f4f4f5] dark:bg-[#27272a] text-[#737373] dark:text-[#a1a1aa]"}`}
              >
                {isPrivate ? (
              ...
              <div>
                <span
                  className={`block text-[14px] font-semibold ${isPrivate ? "text-[#0a0a0a] dark:text-white" : "text-[#0a0a0a] dark:text-white"}`}
                >
                  Make channel private
                </span>
                <span className="block text-[12px] text-[#737373] dark:text-[#a1a1aa]">
            ...
            {isPrivate && members.length > 0 && (
              <div className="mt-2 flex flex-col gap-2">
                <label className="text-[13px] font-semibold text-[#0a0a0a] dark:text-white">
                  Select Members
                </label>
                <div className="max-h-[160px] overflow-y-auto rounded-xl border border-[#efefef] dark:border-white/10 p-2">
                  {members.map((m: any) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`flex h-4 w-4 items-center justify-center rounded border ${selected ? "border-[#0a0a0a] bg-[#0a0a0a] text-white dark:border-white dark:bg-white dark:text-[#0a0a0a]" : "border-[#d4d4d8] dark:border-[#3f3f46] text-transparent"}`}
                        >
